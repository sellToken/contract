...
contract SellToken is Ownable {
    ...
    function getMyPriceSell(address too) view public returns(address[] memory add,uint[] memory,uint[] memory,uint[] memory){
        add=mySells[too].coin;
        uint[] memory routePath = new uint[](add.length);
        uint[] memory routetoken = new uint[](add.length);
        uint[] memory routetokenUSDT = new uint[](add.length);
        //getToken3Price
        if(add.length == 0){
           return (add,routePath,routetoken,routetokenUSDT); 
        }
        for(uint i=0;i< add.length;i++){
            if(Short[too][add[i]].bnb > 0){
               (,,uint istoken)=getShortPrice(too,add[i]);
               (uint ust,uint rot)=getShortPrice1(too,add[i],istoken);
               routePath[i]=istoken;
               routetokenUSDT[i]=ust;
               routetoken[i]=rot;  
            }    
        }
        return (add,routePath,routetoken,routetokenUSDT); 
    ...
    function getShortPrice1(address my,address token0,uint ist) private view returns (uint,uint) {
        return (getToken3Price(token0,Short[my][token0].token),getTokenPrice(token0,Short[my][token0].token,ist));
    }
